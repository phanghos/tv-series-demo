 ...
 */
class GetSimilarTvShowsUseCaseImplTest {
    ...
    fun setUp() {
        initMocks(this)
        useCase = GetSimilarTvShowsUseCaseImpl(repository)
    }
    ...
    fun getSimilarTvShows_should_return_expected_value() {
        ...
        `when`(repository.getSimilarTvShows(0, 1)).thenReturn(expectedObservable)

        // When
        val resultObservable = repository.getSimilarTvShows(0, 1)

        // Then
        verify(repository).getSimilarTvShows(0, 1)
